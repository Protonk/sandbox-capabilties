...
## Layout
- `Makefile` knows how to list, run, and fan out to every `probes/*.py` or `probes/*.c` probe. All artefacts are stored in `artifacts/` (ignored by git).
...
- Individual probe sources live beside the runners (`filesystem_tmp_write.py`, `filesystem_root_write.py`, `process_basic_spawn.py`, `filesystem_tmp_write_c.c`). Each focuses on a single behavior.
...
2. Choose a short slug (e.g., `process_basic_spawn`). Use it as the filename and capability identifier, then pick a language: `probes/<slug>.py` for Python (import `_runner`) or `probes/<slug>.c` for native code (include `_runner_c.h`).
...
## Tests
- Run `make test` before and after meaningful changes to ensure both the Python and C helpers remain stable. The target runs `unittest` plus any `tests/c/*.c` binaries.
- Python tests live under `tests/` as `test_*.py` files that subclass `unittest.TestCase`. Native smoke tests live under `tests/c/` and should link against `_runner_c.c`.
